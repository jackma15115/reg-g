...
class XConsoleAuthClient:
    ...
    def load_signup_page(self) -> int:
        ...
        if status >= 400:
            ...
            raise RuntimeError(
                f"sign-up page returned HTTP {status}; cannot scrape Next.js "
                f"action data. This usually means the current x.ai egress was "
                f"blocked or challenged before the real sign-up page loaded. "
                f"body={preview!r}"
            )
    ...
    def _scrape_action_id(self, html: str) -> str:
        ...
        if not js_urls:
            rsc_count = len(self._RSC_PUSH_RE.findall(html))
            preview = re.sub(r"\s+", " ", html).strip()[:300]
            raise RuntimeError(
                "no Next.js JS chunks found on sign-up page "
                f"(rsc_segments={rsc_count}). The response is not the expected "
                f"x.ai sign-up app, so action scraping cannot continue. "
                f"body={preview!r}"
            )
        ...
        if workers <= 0:
            raise RuntimeError("no JS chunks available to search for the server action ID")
    ...
    def obtain_session_via_password(
        ...
        *,
        ...
        """Password protocol login via AuthManagement/CreateSession → SSO JWT.

        One Turnstile token is single-use and short-lived. Callers that need
        multiple attempts must mint a **fresh** token between calls. Reusing a
        rejected token (especially after ``failed to verify … turnstile``)
        only burns retries and never recovers.
        """
        import time as _time
        ...
        turnstile = str(turnstile_token or "").strip()
        if not email_raw or not password_n or not turnstile:
            self.last_session_diagnostic = "CreateSession 参数不完整"
            return None

        # Prefer lower-case first — xAI accounts are almost always lower-cased.
        email_candidates: list[str] = []
        for e in (email_raw.lower(), email_raw):
        ...
        # Warm sign-in page so CF cookies settle. Do this immediately before
        # CreateSession — the token must still be used in the same second.
        try:
            self._request(
                "GET",
                ref,
        ...
        # Default 1: one token → one CreateSession burst. Higher values only
        # make sense for transient NOT_FOUND / UNAVAILABLE, never for a dead token.
        attempts = max(1, int(retries))
        last_status = None
        last_grpc = None
        last_msg = ""
            ...
            for em in email_candidates:
                ...
                if self.debug:
                    print(
                        ...
                        f"body_len={len(raw or b'')} token_len={len(turnstile)}"
                    )
                ...
                msg_l = str(grpc_msg or "").lower()
                # Token is dead — stop immediately so the caller re-solves captcha.
                if any(
                    k in msg_l
                    for k in (
                        "turnstile",
                    ...
                    token_dead = True
                    if self.debug:
                        print("  [sso] CreateSession turnstile rejected; stop reusing token")
                    break
                # Wrong password / unknown account: no point retrying same token.
                if any(
                    k in msg_l
                    for k in (
                        "invalid-credentials",
                ...
                ) and em == email_candidates[-1]:
                    token_dead = True
                    break

                # NOT_FOUND / UNAVAILABLE: account may still be propagating.
                if grpc_status in (5, 14, None) and attempt < attempts and not token_dead:
                    continue
            if attempt < attempts and not token_dead:
        ...
        message = re.sub(r"[\r\n]+", " ", str(last_msg or "")).strip()[:180]
        detail = f"CreateSession 未返回 SSO（HTTP {last_status or '-'}，gRPC {last_grpc if last_grpc is not None else '-'}"
        if message:
            detail += f"，原因：{message}"
        self.last_session_diagnostic = detail + "）"
        if self.debug:
            print(f"  [sso] {self.last_session_diagnostic}")
        return None
