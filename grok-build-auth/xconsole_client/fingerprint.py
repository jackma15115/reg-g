...
The default impersonate target is `chrome145` (newest stable preset available
in current `curl_cffi` that still passes accounts.x.ai Cloudflare checks).
Older presets such as `chrome131` now receive CF "Attention Required" 403 on
this egress. Surface User-Agent / sec-ch-ua headers come from
`xconsole_client.config` and must stay version-aligned with the impersonate
target.
...
# Defaults that match the current Chrome 145 / Windows profile.
# chrome131 and older are currently CF-challenged on accounts.x.ai from this
# datacenter egress; chrome136+ still loads the real sign-up page.
