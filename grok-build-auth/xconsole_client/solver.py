    ...
    def solve_turnstile(
        ...
        *,
        ...
        Args:
            ...
            proxy: Kept for API compatibility. Local Camoufox is always Proxyless
                (original register-lite behavior); remote YesCaptcha Proxyless
                workers use their own IP and ignore this field.
        ...
        # Original behavior: never force local Camoufox through registration proxy.
        # Protocol HTTP still uses job proxy; captcha uses server egress.
        _ = (proxy or "").strip() or None
        ...
        for idx, task_type in enumerate(task_types):
            ...
            try:
                self._progress(
                    ...
                    + (" proxy=no" if local else " proxy=remote-worker")
                )
